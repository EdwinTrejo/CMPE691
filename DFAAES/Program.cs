...
namespace DFAAES
{
    ...
    public class AESBlock : Constants
    {
        ...
        {
            AESBlock CXORD = XorBlocks(fault_free, faulty);
            AESBlock inverse_CXORD = InverseShiftRows(CXORD);


            AESBlock remove_k10 = XorBlocks(fault_free, faulty);
            AESBlock undo_subbyte_m9 = InverseSubBytes(remove_k10);
            AESBlock subbyte_m9 = InverseSubBytes(fault_free);
            AESBlock possible_key = XorBlocks(subbyte_m9, undo_subbyte_m9);
            return possible_key;
        }
        ...
        private static AESBlock XorBlocks(AESBlock block_A, AESBlock block_B)
        {
            AESBlock backwards_operation = new AESBlock();
            for (int row = 0; row < ROW_SIZE; row++)
            {
